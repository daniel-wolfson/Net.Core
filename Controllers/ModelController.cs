...
using ConvertionTableData = Crpm.Model.Data.ConvertionTableData;
using ModelComponentStatus = Crpm.Model.Data.ModelComponentStatus;
...
namespace Crpm.Controllers
{
    ...
    public class ModelController : ControllerBaseAction
    {
        ...
        {
            var res = await _modelsService.GetModelAffectsThresholds(modelComponentGuid, isRoot);
            return await Task.FromResult(res);
        }
        ...
        #region InterFace
        [HttpPost("SaveModelComponentByInterFace")]
        public async Task<List<string>> SaveModelComponentByInterFace([FromQuery] string user_guid, [FromQuery] string model_component_parent_guid, [FromBody] List<ModelData> model_data, [FromQuery] bool interfaceFlag = true)
        {
            List<string> guid_list = new List<string>();


            string guid;
            var tempSubjectId = string.Empty;
            var tempParentGuid = string.Empty;
            tempSubjectId = model_data[0].model_component_guid;
            model_data[0].model_component_guid = "";

            guid = await _modelsService.SaveModelComponent(user_guid, model_component_parent_guid, model_data[0], interfaceFlag);
            guid = guid + "@" + tempSubjectId;
            guid_list.Add(guid);
            //await _modelsService.OkResult(guid);

            var orderMD_List = model_data.GroupBy(x => x.parent_guid).ToList();

            foreach (var groupMD in orderMD_List)
            {
                foreach (ModelData MD in groupMD)
                {
                    ...
                    tempParentGuid = guid_list.Where(x => x.Split("@")[1] == groupMD.Key).FirstOrDefault().Split("@")[0];

                    tempSubjectId = MD.model_component_guid;
                    MD.model_component_guid = "";
                    model_component_parent_guid = tempParentGuid;
                    guid = await _modelsService.SaveModelComponent(user_guid, model_component_parent_guid, MD, interfaceFlag);
                    guid = guid + "@" + tempSubjectId;
                    guid_list.Add(guid);
                    //await _modelsService.OkResult(guid);
            ...
            await CanActivateModel(guid_list[0].Split("@")[0], true); // active the model after created

            await _modelsService.OkResult();

            return guid_list;
        }


        [HttpPost("SaveOutSourceScore")]

        public async Task<bool> SaveOutSourceScore([FromBody] List<OutSourceScore> _outSource_Scores)
        {
            var res = await _modelsService.SaveOutSourceScoreToDb(_outSource_Scores);
            await _modelsService.OkResult();
            return res;
        }
        #endregion
